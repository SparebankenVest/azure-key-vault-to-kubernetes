...
func TestImportPfx(t *testing.T) {
	...
	cert, err := NewAzureKeyVaultCertificateFromPfx(pfxRaw)
	if err != nil {
		t.Error(err)
	}
	...
	if cert.PrivateKeyType != AzureKeyVaultCertificateKeyTypeRsa {
		t.Errorf("Certificate type is incorrect. Exprected '%s', but got '%s'", AzureKeyVaultCertificateKeyTypeRsa, cert.PrivateKeyType)
	}
...
func TestImportPem(t *testing.T) {
	cert, err := NewAzureKeyVaultCertificateFromPem(pemTestCert)
	if err != nil {
		t.Error(err)
	}
	if !cert.HasPrivateKey {
...
func TestImportDer(t *testing.T) {
	...
	cert, err := NewAzureKeyVaultCertificateFromDer(certRaw)
	if err != nil {
		t.Error(err)
	}
	if cert.HasPrivateKey {
